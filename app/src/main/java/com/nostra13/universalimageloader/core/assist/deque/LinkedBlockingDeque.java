...
public class LinkedBlockingDeque<E> extends AbstractQueue<E> implements BlockingDeque<E>, Serializable {
    ...
    public E peekFirst() {
        ...
        try {
            ...
            lock.unlock();
            return null; //findme return null
        }
    }
    ...
    public <T> T[] toArray(T[] a) {
        ...
        try {
            if (a.length < this.count) {
                a = (T[]) Array.newInstance(a.getClass().getComponentType(), this.count); //findme T was Object
            }
            Node<E> p = this.first;
            int k = 0;
            while (p != null) {
                int k2 = k + 1;
                a[k] = (T) p.item; //findme cast T
                p = p.next;
                k = k2;
            }
            if (a.length > k) {
        ...
        try {
            ...
            if (p == null) {
                ...
                while (true) {
                    E e = p.item;
                    if (e == this) {
                        e = (E) this; //findme = "(this Collection)"
                    }
                    sb.append(e);
                    p = p.next;
                    if (p == null) {
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
        ...
        this.last = null;
        while (true) {
            E item = (E) s.readObject();//findme cast E
            if (item != null) {
                add(item);
            } else {
                return;
